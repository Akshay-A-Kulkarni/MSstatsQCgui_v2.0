...
maindf <- rbind(sample_frac(pass,0.95),sample_frac(pass,0.05)) %>% select(-peptide)
...
n = length(isoforest_result[,6])/2
B = 10000
result = rep(NA, B)
for (i in 1:B) {
  boot.sample = sample(n, replace = TRUE)
  result[i] = quantile(isoforest_result[,6][boot.sample], 0.90)
}
thresh<-mean(result)
```
...
maindf <- rbind(sample_frac(pass,0.5),sample_frac(fail,0.5)) %>% select(-peptide)
...
qplot(isoforest_result$Anom_Score, geom="density") 

#appending output to input df 
isoforest_result
table(isoforest_result$RESPONSE, isoforest_result$Predicted_Class)
...
q<-quantile(results$anomaly_score[1:900]
         , probs = seq(0.9, 0.9, length.out = 1)
         )
outlier<-as.factor(ifelse(results$anomaly_score >=q, "FAIL", "PASS"))
...
plot_pca(isoforest_result[c(1:4,6)])
```
