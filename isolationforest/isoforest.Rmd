...
# testdf <- input_df[-maindf$X,]
# input_df <- read.csv("~/Desktop/GitHub/MSstatsQCgui_v2.0/isolationforest/test_maindf.csv")


# maindf <-  input_df[2:6]
# write.csv(maindf,'solitude_test_1000.csv')
...
iso <- isolationForest$new(
                           ...
                           replace = T,
                           mtry = (length(colnames(maindf[,1:4]))),
                           seed = 123
                           )
...
res2 <- iso$predict(testdf[2:5])


outlier<-as.factor(ifelse(res2$anomaly_score > thresh, "FAIL", "PASS"))
res2<-cbind(res2, outlier, RESPONSE=testdf$RESPONSE)

table(res2$RESPONSE, res2$outlier)
